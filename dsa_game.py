"""
DSA Adventure - Game Module
============================
Data Structures & Algorithms learning game with levels and boss battles.
"""
...
from game_arcade_ui import render_game_arcade, render_game_selector_compact
from game_library import get_games_by_category

# DSA Game Configuration
GAME_ID = "dsa"
GAME_TITLE = "DSA ADVENTURE"
GAME_ICON = "DSA"
GAME_COLOR_1 = "#ff6b6b"
GAME_COLOR_2 = "#ffd93d"
...
# Level definitions
LEVELS = [
    {"id": 1, "name": "Arrays", "icon": "01", "topic": "Arrays in Data Structures"},
    {"id": 2, "name": "Linked Lists", "icon": "02", "topic": "Linked Lists"},
    {"id": 3, "name": "Stacks & Queues", "icon": "03", "topic": "Stacks and Queues"},
    {"id": 4, "name": "Trees", "icon": "04", "topic": "Binary Trees and BST"},
    {"id": 5, "name": "Graphs", "icon": "05", "topic": "Graph Data Structures and Algorithms"},
    {"id": 6, "name": "BOSS BATTLE", "icon": "BOSS", "topic": "Advanced DSA Concepts", "is_boss": True}
]

# Map each level to its corresponding mini-game
LEVEL_TO_GAME_MAPPING = {
    1: "array-sorter",      # Arrays -> Array Sorter
    2: "link-builder",      # Linked Lists -> Link Chain Builder
    3: "stack-tower",       # Stacks & Queues -> Stack Tower
    4: "tree-builder",      # Trees -> Binary Tree Builder
    5: "graph-explorer",    # Graphs -> Graph Path Explorer
    # Boss Battle will have a special challenge
}
...
def inject_game_css():
    ...
    <style>
        ...
        .stApp {{
            background: linear-gradient(180deg, #1a0a0a 0%, #2d1a1a 50%, #1a1a2e 100%);
        }}
        ...
        .path-line {{
            ...
            bottom: 0;
            width: 4px;
            background: linear-gradient(180deg, {GAME_COLOR_1}44, {GAME_COLOR_2}44);
        ...
        /* Boss Battle styling */
        .boss-container {{
            background: linear-gradient(135deg, #2d1a1a, #1a1a2e);
...
def render_learning_path():
    ...
    for i, level in enumerate(LEVELS):
        ...
        # Determine if level is unlocked (previous level completed or first level)
    ...
    with col3:
        ...
        if st.button(f"{game_icon} {game_name.upper()}", use_container_width=True, type="secondary"):
            if level_game:
                ...
                st.session_state.show_arcade = True
            st.session_state.show_quiz = False
            st.session_state.explanation_result = None
            st.rerun()
    
    # Show game arcade if active
    if st.session_state.get('show_arcade', False):
        st.markdown("---")
        st.markdown(f"""
            <div style="text-align: center; margin: 20px 0;">
        ...
        # Show game arcade
        render_game_arcade("DSA", GAME_COLOR_1)
    
    # Show mini-game if active
    elif st.session_state.get('show_mini_game', False) and st.session_state.get('selected_mini_game'):
        st.markdown("---")
        mini_game_id = st.session_state.selected_mini_game
        available_games = get_available_mini_games()
        mini_game_info = available_games.get(mini_game_id)
        
        if mini_game_info is None:
            st.warning(f"Game '{mini_game_id}' not found. Please select another game.")
            st.session_state.show_mini_game = False
            st.session_state.selected_mini_game = None
        else:
            st.markdown(f"""
...
def render_boss_battle(level):
    ...
    if st.button("CHALLENGE THE BOSS", type="primary", use_container_width=True):
        with st.spinner("The boss awakens..."):
    ...
    if hasattr(st.session_state, 'boss_question') and st.session_state.boss_question:
        ...
        st.markdown(f'''
            <div class="quiz-question" style="border-color: #ff4444;">
                <div class="quiz-question-number" style="color: #ff4444;">BOSS CHALLENGE</div>
...
def render_dsa_game():
    """Main render function for DSA game."""
    inject_game_css()
    render_game_header()
