"""
Database Quest - Game Module
============================
Database Management Systems learning game with levels and boss battles.
"""
...
# DBMS Game Configuration
GAME_ID = "dbms"
GAME_TITLE = "DATABASE QUEST"
GAME_ICON = "SQL"
GAME_COLOR_1 = "#00d4ff"
GAME_COLOR_2 = "#7c3aed"
...
# Level definitions
LEVELS = [
    {"id": 1, "name": "SQL Basics", "icon": "01", "topic": "SQL Basics and Queries"},
    {"id": 2, "name": "Normalization", "icon": "02", "topic": "Database Normalization (1NF, 2NF, 3NF, BCNF)"},
    {"id": 3, "name": "ER Diagrams", "icon": "03", "topic": "Entity Relationship Diagrams and Modeling"},
    {"id": 4, "name": "Transactions", "icon": "04", "topic": "Database Transactions and ACID Properties"},
    {"id": 5, "name": "Indexing", "icon": "05", "topic": "Database Indexing and Query Optimization"},
    {"id": 6, "name": "BOSS BATTLE", "icon": "BOSS", "topic": "Advanced DBMS Concepts", "is_boss": True}
]

# Map each level to its corresponding mini-game
LEVEL_TO_GAME_MAPPING = {
    1: "sql-detective",           # SQL Basics -> SQL Detective
    2: "sql-detective",           # Normalization -> SQL Detective
    3: "schema-builder",          # ER Diagrams -> Schema Builder
    4: "transaction-simulator",   # Transactions -> Transaction Simulator
    5: "query-optimizer",         # Indexing -> Query Optimizer
...
def inject_game_css():
    ...
    <style>
        ...
        .stApp {{
            background: linear-gradient(180deg, #0a0a1a 0%, #1a1a3e 50%, #0d1a2e 100%);
        }}
        ...
        @keyframes bossGlow {{
            0%, 100% {{ box-shadow: 0 0 20px {GAME_COLOR_2}44; }}
            50% {{ box-shadow: 0 0 50px {GAME_COLOR_2}aa; }}
        }}
...
def render_boss_battle(level):
    ...
    st.markdown(f'''
        <div class="boss-container">
            ...
            <div class="boss-subtitle">Defeat the Database Wizard!</div>
    ...
    if st.button("CHALLENGE THE BOSS", type="primary", use_container_width=True):
        with st.spinner("The wizard awakens..."):
    ...
    if hasattr(st.session_state, 'boss_question') and st.session_state.boss_question:
        ...
        st.markdown(f'''
            <div class="quiz-question" style="border-color: {GAME_COLOR_2};">
                <div class="quiz-question-number" style="color: {GAME_COLOR_2};">BOSS CHALLENGE</div>
...
def render_dbms_game():
    """Main render function for DBMS game."""
    inject_game_css()
    render_game_header()
